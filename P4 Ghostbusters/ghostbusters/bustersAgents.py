...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        #print("GHOST: ", livingGhosts)
        
        dist = 1000000000
        minPos = (1,1)
        
        live = sum(livingGhosts)
        #print("LIVE", live)
        
        if live > 0:
            for ghostP in livingGhostPositionDistributions:
                pos2 = ghostP.argMax()
                tmpDist = self.distancer.getDistance(pacmanPosition, pos2)
                if tmpDist < dist:
                    dist = tmpDist
                    minPos = pos2
            ...
            for action in legal:
                ...
                tmpDist = self.distancer.getDistance(successorPosition, minPos)
                if tmpDist < dist:
                    dist = tmpDist
                    finalAction = action
